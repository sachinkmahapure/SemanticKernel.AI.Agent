    ...
    ILogger<ConversationService> logger)
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var exists = await db.ChatSessions.AnyAsync(s => s.Id == sessionId, ct);
            if (exists)
            {
                await db.ChatSessions
                    .Where(s => s.Id == sessionId)
                    .ExecuteUpdateAsync(s =>
                        s.SetProperty(x => x.LastActivityAt, DateTimeOffset.UtcNow), ct);
                return sessionId;
            }
        }
    ...
    public async Task AddUserMessageAsync(string sessionId, string content, CancellationToken ct = default)
    {
        await AddMessageAsync(sessionId, AppConstants.Roles.User, content, ct);
    }

    /// <summary>Persist an assistant message to the database.</summary>
    public async Task AddAssistantMessageAsync(string sessionId, string content, int? tokens = null, CancellationToken ct = default)
    {
        await AddMessageAsync(sessionId, AppConstants.Roles.Assistant, content, ct, tokens);
    }
    ...
    private async Task AddMessageAsync(
        ...
        CancellationToken ct, int? tokens = null)
    {
